...
try:
    ...
    time.sleep(2)                                           spinner.fail('Wrong URL or Empty Field')
    spinner.stop()
...
def get_file_info_api(file_id):                             url = f"https://api.anonfiles.com/v2/file/{file_id}/info"                                                       response = s.get(url)
    data = response.json()                                  return data
                                                        def generate_ids(chars, length, number):
    ids = []                                                for i in range(number):
        id = ''
